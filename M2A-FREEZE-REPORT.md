...
| Gate | 状态 | 依据 |
|---|---|---|
| G1 supersede 原子性 | PARTIAL | 新 manifest 先落盘、旧 outcome 补写、只允许 supersede 冲突 head；缺两阶段间崩溃测试 |
| G2 祖先清理 | PARTIAL | cleanupTerminal 校验图 + 祖先 txid + tombstone；缺逐祖先崩溃测试 |
...
| G6 测试矩阵自包含 | PARTIAL | 公开 API 生成的 8 场景子进程 crash 矩阵；未穷举全部 durable 边界 |
| G7 schema 完整 | PARTIAL | op/outcome/evidence 有严格校验；manifest/report/plan 仍缺统一版本化 schema |
...
| G10 故障注入闭环 | PARTIAL | deterministic failpoint + 子进程 exit；kill -9 全边界未穷举 |
