...
## 当前状态
- **导航与公开品牌修正（已上线并只读验收）**：Cordis 目录橱窗 Hero 的顶栏
  ...
  父仓库 `9a9f6de` 已由 Zeabur 部署为 RUNNING；1600px 英文 Hero、Hero 滚出后的中性浮层、普通亮/暗
  页面和 390px 移动端截图均通过。`/marketplace`、`/en/marketplace`、`/plugins`、`/en/guides` 均为
  200，SSR/metadata 断言也确认不再公开 `DSH Cordis` 或 `CORDIS / MARKETPLACE HOST`。
- **市场宿主 Hero 六方案与 V5 目录橱窗（已上线）**：父仓库为 Cordis
  ...
  才能访问；未设或其它值一律 404，且 `noindex,nofollow`。正式页只直接加载 V5，避免预览方案进入
  现役首屏依赖。本次选择将 V5 提取为独立组件，正式页直接加载 V5，生产路由不包含其它五版预览标记；
  V5 遗漏的 npm 发现链接已由首轮测试发现并修复。本地已通过 TypeScript、ESLint、390 个 unit tests、
...
- **市场宿主公开产品面（已上线）**：父仓库已实现 Cordis 专属
  ...
  有文件级、已说明的 exports 豁免：它保留 `.env.local` 先于数据库模块初始化的 fail-closed 加载顺序。
  生产 Zeabur 已部署父仓库 `f113cb2`；`https://cordis.run/marketplace`、`/en/marketplace`、首页介绍区及
  新版 64×64 SVG 均以只读 HTTP `200` 验收，canonical/alternate 正确，非 Cordis 的
  `https://mifar.net/marketplace` 返回 404。npm `0.1.1`
